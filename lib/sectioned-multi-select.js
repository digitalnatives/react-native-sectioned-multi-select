...
class SectionedMultiSelect extends PureComponent {
  ...
  _removeItemOnBackspace = (event, text, items) => {
    ...
    const selectedItem = items
      .map(item =>
        item.availableActivities.filter(
  ...
  _displaySelectedItems = () => {
    ...
    const selected = selectedItems.map(item => this._findItem(item));
    const allItem = selectedActivitiesObject.concat(selected);

    return allItem.map(singleSelectedItem => {
      ...
      if (item.id === runningInsurance.activity_id) {
